            ...
            for changed_file in changed_files:
                for path in path_to_owner.keys():
                    if ("/"+changed_file).startswith(path):
                        ...
                        if the_owner:
                            if commit.author.login not in owners[the_owner]["known_authors"]:
                                owners[the_owner]["known_authors"].add(commit.author.login)
                                owners[the_owner]["authors"].append({"name": commit.author.name, "login": commit.author.login, "url": commit.author.url})

    countries = []
    for owner in owners.keys():
        countries.append({
            "name": owner.lower().replace("@getsentry/", "").replace("owners-", "").replace("team-", ""),
            "full_name": owner,
            "topics": [],
            "url": "",
            ...
            for commit in commits:
                if hasattr(commit, "author") and commit.author is not None:
                    if commit.author.login not in known_authors:
                        ...
                        authors.append({"name": commit.author.name, "login": commit.author.login, "url": commit.author.url})
